    ...
    <div id="appCapsule">
        ...
        <div class="section pt-1">
            <div class="container mt-3">
                <form action="{{ route('lostItems.update', $lostitem->slug) }}" method="POST"
                        ...
                        <select name="category_id" id="" class="form-control form-custom">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $lostitem->category_id == $category->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="no_tlp" id="" placeholder="Gunakan format 62"
                            class="form-control form-custom" value="{{ $lostitem->no_tlp }}">
                    </div>
                    <div class="mb-3">
                        <label for="">Status</label>
                        <select name="status" class="form-control" id="">
                            <option value="belum" @if ($lostitem->status == 'belum') selected @endif>Belum ditemukan
                            </option>
                            <option value="ditemukan" @if ($lostitem->status == 'ditemukan') selected @endif>Sudah ditemukan
                            </option>
                        </select>
                    </div>
