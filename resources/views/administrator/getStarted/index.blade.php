        ...
        @foreach ($data as $key => $getstarted)
            <div class="modal fade" id="editData{{ $getstarted->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Edit Data Get Started</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{ route('administrator.getStarted.update', ['slug' => $getstarted->slug]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="title" class="form-label">Title</label>
                                ...
                                <tbody>
                                    @foreach ($data as $key => $getstarted)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('administrator.getStarted.destroy', ['slug' => $getstarted->slug])}}" class="btn tombol-hapus btn-danger m-1">
